...
graphPoints::loadPoints(std::string _fileName, sf::Vector2u winSize) {
...
// set positions of graph points from point co-ordinates loaded in from CSV file
   ...
   // {
       // addPoint(sf::Color(255, 0, 0), 1, sf::Vector2u(1, 1), sf::Vector2f(0, 0));
        ...
        //    if (j == 1) points[i].setPosition(sf::Vector2f(points[i].getPosition().x + gap * i, winSize.y - float(stoi(cords[i][j])))); // y coordinate
       // }
    //}
#pragma endregion
